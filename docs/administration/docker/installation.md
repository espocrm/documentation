...
You can deploy EspoCRM using either [Docker Run](#docker-run) or [Docker Compose](#docker-compose). For production environments, we recommend Docker Compose, as it is easier to manage and provides better configuration control. While Docker Run is quicker to set up, it is a good choice if you want to try the system out.
...
Before performing the following steps, be sure to make a [backup](../backup-and-restore.md#docker-installation). By following these instructions, your customizations and data will be preserved.
