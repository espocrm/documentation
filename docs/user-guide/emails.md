...
CSS styles defined in a *<style>* tag are not supported and will be stripped after editing the HTML. HTML documents that use separate CSS styles can be converted to use inline styles (for example, using [this converter](https://templates.mailchimp.com/resources/inline-css/)). If an HTML document generated by another tool contains CSS in *<style>* tags, it can be used in Espo after converting the styles to inline CSS.
