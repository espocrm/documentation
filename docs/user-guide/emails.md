...
An email moved to a group folder is automatically assigned to the group folder's teams and associated with users for those teams, meaning that team users will obtain read access to the email.
...
There's the ability to automatically put inbound emails imported through a group email account to a specific group folder. It is also possible to move emails matching an email filter to a specific group folder (the filter should be related to a group email account).

Users that have access to a group folder can put there emails that they have edit access to. If an email is located in a group folder, and the user does not have edit access to, then they won't be able to move that email to another folder.
...
CSS styles defined in a `<style>` tag are not supported and will be stripped after editing the HTML. HTML templates that use separate CSS styles can be converted to use inline styles (for example, using [this converter](https://templates.mailchimp.com/resources/inline-css/)). If an HTML generated by another email builder tool contains CSS in a *<style>* tag, it can be used in Espo after conversion.
