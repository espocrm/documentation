...
When choosing the *Entity Type* for a grid report, consider the following. The field that will be aggregated by should be the field of the entity type of the report. E.g. if you need to sum by the opportunity amount, choose the Opportunity entity type. If you neeed to aggregate by fields from multiple entity types, consider creating separate grid reports for each entity type and then use a Joint Report to join them into one.
